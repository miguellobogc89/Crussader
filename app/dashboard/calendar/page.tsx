...
/* ===== Types ===== */
type Appointment = { id: string; locationId: string; serviceId: string; startAt: string; endAt: string; status?: "PENDING" | "BOOKED" | "COMPLETED" | "CANCELLED" | "NO_SHOW" | null; employeeId?: string | null; resourceId?: string | null; customerName?: string | null; customerPhone?: string | null; customerEmail?: string | null; notes?: string | null; };
...
type CalendarAppt = { id: string; startAt: string; endAt: string; serviceName?: string | null; serviceColor?: string | null; employeeName?: string | null; resourceName?: string | null; };
type View = "day" | "threeDays" | "workingWeek" | "week" | "month";

export default function CalendarPage() {
  // >>> Altura regulable SOLO para Calendar
  const WORKSPACE_HEIGHT = "78vh"; // ej.: "calc(100dvh - 72px)" si quieres descontar header fijo
  ...
  const fmtDayTitle = useMemo(
    () => new Intl.DateTimeFormat("es-ES", { timeZone: "Europe/Madrid", weekday: "long", year: "numeric", month: "long", day: "2-digit" }),
    []
  );
  ...
  return (
    <SidebarProvider>
      ...
      >
        {/* Workspace SOLO para Calendar, altura regulable */}
        <div className="min-h-0 overflow-hidden" style={{ height: WORKSPACE_HEIGHT }}>
          <div className="flex h-full gap-4 overflow-hidden">
            {/* Izquierda — panel colapsable */}
            ...
            {/* Centro + derecha */}
            <div className="flex-1 min-w-0 flex gap-4 h-full overflow-hidden">
              <CalendarCenter
                selectedView={selectedView}
                setSelectedView={(v: View) => setSelectedView(v)}

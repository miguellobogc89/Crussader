// app/api/reviews/normalized/rank/entities/route.ts
...
export async function GET(req: Request) {
  try {
    ...
    const links = await prisma.concept_normalized_entity.findMany({
      where: { status: "active" },
      select: {
        normalized_entity_id: true,
        concept: {
          select: {
            location_id: true,
            structured: true,
    ...
    for (const l of links) {
      ...
      const cur = bucket.get(l.normalized_entity_id) ?? {
        mentions: 0,
        sumSigned: 0,
        sumIntensity: 0,
        pos: 0,
      ...
      bucket.set(l.normalized_entity_id, cur);
    }
    ...
    const entities = await prisma.normalized_entity.findMany({
      where: { id: { in: ids } },
      select: { id: true, display_name: true, canonical_key: true },
    });

    const rows: RankRow[] = entities
      .map((e) => {
        const b = bucket.get(e.id);
        if (!b) return null;

// app/api/crussader-assistant/conversations/route.ts
...
export async function GET(req: NextRequest) {
  try {
    ...
    const takeRaw = Number(searchParams.get("limit") || 30);
    const take = Math.min(Math.max(takeRaw || 30, 1), 100);
      ...
      conversations.map(async (conversation) => {
        ...
        return {
          ...
          contact: {
            ...
            phone_e164:
              conversation.contact_phone_e164 ?? conversation.contact_external_id,
    ...
    let message = "Error al listar conversaciones";

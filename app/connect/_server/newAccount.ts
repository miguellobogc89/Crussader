...
export async function createNewAccount(params: {
  ...
  const googleAccountId = canonicalGoogleAccountId(googleAccountIdRaw); // ✅ SIN "accounts/"
  ...
  // Mapeo: recibimos resourceName completo, guardamos ID canónico y conservamos el name completo en googleName
  ...
  // ✅ GUARDIA 1: si ya existe google_gbp_account para ese google_account_id (canonical o con prefijo), no es "new"
  ...
  // ✅ GUARDIA 2: si existe Location por googleLocationId, no petamos: la adoptamos si coincide googleAccountId (tolerante prefijo)
  // Si pertenece a otro googleAccountId -> conflicto
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const gbp = await tx.google_gbp_account.create({
      data: {
        ...
        google_account_id: googleAccountId, // ✅ SIN "accounts/"
    ...
    for (const l of incomingLocations) {
      ...
      if (existingLoc?.id) {
        ...
        if (existingLoc.googleAccountId && !sameGoogleAccountId(existingLoc.googleAccountId, googleAccountId)) {
          return {
            ok: false,
            error: "location_conflict",
            detail: {
        ...
        const updated = await tx.location.update({
          ...
          data: {
            ...
            googleLocationId: google_location_id, // canonical
      ...
      // Guardamos ID canónico en google_location_id y el resourceName completo en google_location_name

// app/components/admin/integrations/whatsapp/templates/useWhatsAppTemplatesAdmin.ts
"use client";
...
export function useWhatsAppTemplatesAdmin(companyId: string | null) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/whatsapp/templates?companyId=${encodeURIComponent(companyId)}`,
        { cache: "no-store" }
      );

// app/components/crussader/UX/table/DataTable.tsx
"use client";
 ...
 * - (Nuevo) Fila expandible inline mediante renderExpandedRow.
 */
export default function DataTable<T>({
  ...
  // NUEVO: expansión
  expandedRowIds = [],
  onToggleExpand, // la usa el padre; aquí no la llamamos
  renderExpandedRow,
}: {
  columns: ColumnDef<T>[];
  rows: T[];
  ...
  return (
    <Card className={className}>
      <CardContent className="p-0">
        {/* Toolbar: buscador global */}
        {showGlobalSearch && (
          <div className="flex items-center justify-end px-4 py-3">
            <SearchBar value={qGlobal} onChange={setQGlobal} placeholder={searchPlaceholder} />
          </div>
        )}
        ...
        <table className="w-full text-sm border-t" style={{ tableLayout: "fixed" }}>
            ...
            <tr>
              {columns.map((c, idx) => (
                <HeaderCell
                  ...
                  setTextFilterValue={
                    c.filter === "text" ? (v) => setTextFilters((prev) => ({ ...prev, [c.id]: v })) : undefined
                  }
                  booleanFilterValue={c.filter === "boolean" ? (boolFilters[c.id] ?? null) : undefined}
                  setBooleanFilterValue={
                    c.filter === "boolean" ? (v) => setBoolFilters((prev) => ({ ...prev, [c.id]: v })) : undefined
                  }
                />
              ))}
              {withActions && <th className="px-2 py-2 font-semibold text-center bg-muted/60 w-[4%]" />}
            </tr>
          </thead>
          ...
          <tbody>
            {processed.length === 0 ? (
              <tr>
                <td className="px-3 py-6 text-muted-foreground text-center" colSpan={columns.length + (withActions ? 1 : 0)}>
                  Sin resultados
                </td>
              </tr>
            ) : (
              processed.map((row) => {
                ...
                return (
                  <React.Fragment key={key}>
                      ...
                      {columns.map((c, idx) => {
                        ...
                        return (
                          <td
                            ...
                            title={typeof c.accessor(row) === "string" ? (c.accessor(row) as string) : undefined}
                          >
                    ...
                    {/* Fila expandida */}
                    {renderExpandedRow && isExpanded(key) && (
                      <tr className="border-t bg-muted/30">
                        <td className="px-3 py-3" colSpan={columns.length + (withActions ? 1 : 0)}>

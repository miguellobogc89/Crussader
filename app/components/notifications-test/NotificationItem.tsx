    ...
    <li>
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-3">
            <h3 className={cn("truncate text-sm font-medium", isUnread ? "text-foreground" : "text-muted-foreground")}>
              {title}
            </h3>
            {relative && (
          ...
          <p className="truncate text-sm text-muted-foreground">{rawBody}</p>
        </div>

        {/* Acciones al hover (opcional) */}
        {/* ... si quieres un botón de menú contextual, lo metemos aquí ... */}
      </div>
      ...
      >
        <div className="px-4 pb-3 pt-2">
          {bodyHeader && (
            <p className="text-sm leading-relaxed text-foreground/90">{bodyHeader}</p>
          )}
          {commentLine && (
            <p className="ml-4 text-sm italic text-muted-foreground">“{commentLine}”</p>
          )}
          ...
          <div className="mt-2 flex justify-end gap-2">
            ...
            >
              {isUnread ? <Mail className="h-3.5 w-3.5" /> : <MailOpen className="h-3.5 w-3.5" />}
              {!isUnread && "No leída"}
            </button>
            <button
              type="button"

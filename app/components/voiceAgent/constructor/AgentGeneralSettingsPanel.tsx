...
function DiscreteSlider({
  ...
  onChange,
  disabled,
  steps = 5,          // <-- NUEVO (por defecto 5 posiciones: 0..4)
}: {
  stepValue: number;
  ...
  return (
    <div className="flex items-center gap-2">
      <input
        ...
        max={max}       // <-- usa steps-1
        step={1}
        value={Math.min(stepValue, max)}
        onChange={(e) => onChange(Number(e.target.value))}
        disabled={disabled}
        className="w-full accent-indigo-600 disabled:opacity-50"
      />
      <div className="grid gap-1 text-[10px] text-slate-500" style={{gridTemplateColumns:`repeat(${steps}, minmax(0,1fr))`}}>
        {Array.from({length: steps}).map((_,i)=><span key={i}>{i}</span>)}
      </div>
    </div>
  );
}
...
export default function AgentGeneralSettingsPanel({
  ...
  // mapeos discretos
  const MAP = {
    ttsRate: [0.7, 1.0, 1.3, 1.6] as const,
